...
for file in $input_ER; do
	...
	echo $unknown_path/rdf/ER-std/${file%.json}.nt

    echo "[CONFIGURATION]" > config.ini
    echo "output_file: $unknown_path/rdf/ER-std/${file%.json}.nt" >> config.ini
    echo "output_format: N-TRIPLES" >> config.ini
    echo "[X5GON]" >> config.ini
    echo "mappings: $unknown_path/mapper/ER-std/ER-std_rules.ttl" >> config.ini
